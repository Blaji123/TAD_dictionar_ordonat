...
void testAdaugaInexistente(){
    ...
    DO d_suplimentar = DO(cresc);
    int cMin_suplimentar = 0;
    int cMax_suplimentar = 150;
    populeazaDOVidIdentic(d_suplimentar, cMin_suplimentar, cMax_suplimentar);
    assert(d.adaugaInexistente(d_suplimentar)==100);
    assert(d.dim() == (cMax - cMin_suplimentar + 1));
    for(int c=cMin_suplimentar;c <= cMax; c++){
        assert(d.cauta(c)==c);
    }
}
